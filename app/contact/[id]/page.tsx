...
export default async function ContactPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const waClass = "flex items-center justify-center gap-2 rounded-xl bg-green-600 p-4 font-semibold text-white transition-colors hover:bg-green-700";
    ...
    <div className="mx-auto max-w-xl px-4 py-10">
      ...
      <div className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Votre CNI a ete retrouvee a <strong>{match.found.foundCity}</strong>.
        </p>

        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <a href={`tel:${match.found.phone}`} className={callClass}>Appeler</a>
          {whatsappDigits && (
            <a href={`https://wa.me/${whatsappDigits}`} target="_blank" rel="noopener noreferrer" className={waClass}>WhatsApp</a>
          )}
        </div>
        ...
        <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700">
          ...
          citoyenne independante.
        </div>

        <a href="/signaler" className="mt-4 block text-center text-sm text-red-600 hover:underline">
